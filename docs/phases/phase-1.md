...
## 회고
**무엇을 배웠나**
- **계층별 포트 격리가 병렬 프로젝트의 실전 이슈**: 회사 프로젝트와 DB(5432), API(3000), 프론트(5173/5174)가 전부 충돌 → 5433/3001/5175로 이동. 세 번 다 같은 패턴.
...
- **격리된 개인 gh(GH_CONFIG_DIR)**로 회사 계정을 안 건드리고 터미널에서 Issue→PR→CI→merge 전부 자동화.
