...
// Phase 3 S1(+P4 인증): 태그 N:M 연결/조회/재사용.
afterAll(async () => {
  await closeDb()
})

async function create(app: FastifyInstance, h: Record<string, string>, payload: Record<string, unknown>) {
...
describe('tags N:M — P3-S1', () => {
  it('POST /posts with tags → GET /posts/:id에 tags 포함', async () => {
    ...
    const p = await create(app, h, { title: '태그글', tags: ['drizzle', 'fastify'] })
    const res = await app.inject({ method: 'GET', url: `/posts/${p.id}`, headers: h })
  ...
  it('PATCH tags → 링크 교체', async () => {
    const app = buildApp()
    const h = auth(await registerAndToken(app))
    const p = await create(app, h, { title: 'x', tags: ['a', 'b'] })
    await app.inject({ method: 'PATCH', url: `/posts/${p.id}`, headers: h, payload: { tags: ['c'] } })
    const res = await app.inject({ method: 'GET', url: `/posts/${p.id}`, headers: h })
  ...
  it('중복 태그는 재사용(tags 테이블에 1행)', async () => {
    ...
    await create(app, h, { title: 'one', tags: [name] })
    await create(app, h, { title: 'two', tags: [name] })
